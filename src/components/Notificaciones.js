...
const Notificaciones = () => {
  ...
  return (
    <div className="relative" ref={notificacionesRef}>
      <svg onClick={toggleNotificaciones} xmlns="http://www.w3.org/2000/svg" height={'19'} width={'20'} className='mr-3 cursor-pointer fill-blue-500' viewBox="0 0 448 512">
        ...
        <div className="absolute right-0 mt-2 w-80 bg-white shadow-lg rounded-lg overflow-hidden">
          ...
          <div className="flex space-x-2 p-2 border-b">
            <button 
              ...
              className={`px-3 py-1 rounded-full text-sm ${filtro === 'todas' ? 'bg-blue-500 text-white' : 'text-gray-700'}`}
            >
              Todas
            </button>
            <button 
              onClick={() => setFiltro('no_leidas')}
              className={`px-3 py-1 rounded-full text-sm ${filtro === 'no_leidas' ? 'bg-blue-500 text-white' : 'text-gray-700'}`}
            >
              No leídas
            </button>
          </div>
